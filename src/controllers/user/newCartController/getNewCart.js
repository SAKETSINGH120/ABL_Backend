  ...
  try {
    ...
    if (defaultAddress) {
      ...
      return res.status(400).json({
        success: false,
        message:
          "No delivery location found (default address or user location missing)",
      });
    }
    ...
    cart.shops = await Promise.all(
      cart.shops.map(async (shop) => {
        ...
        if (isNaN(shopLat) || isNaN(shopLong)) {
          console.error(
            `Invalid shop coordinates for shop ${shop.shopId?._id}:`,
            {
              lat: shop.shopId?.lat,
        ...
        const { distanceKm, durationText, deliveryCharge } =
        ...
        shopBreakdown.push({
          ...
          deliveryMessage,
          shopTotal: Number(
            (shopItemTotal + packingCharge + deliveryCharge).toFixed(2)
          ),
        });
    ...
    const gst = Math.ceil(
      (subtotal + totalPackingCharge + totalDeliveryCharge + platformFee) * 0.18
    );
    const grandTotal =
      subtotal + totalPackingCharge + totalDeliveryCharge + platformFee + gst;
